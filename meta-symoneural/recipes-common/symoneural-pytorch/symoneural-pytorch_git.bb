...
SYMON_HOOKS_DIR = "${WORKDIR}/symoneural-hooks"
# The interpreter the PeachPy wrapper delegates to; matches what
# FindPythonInterp resolved before the wrapper was interposed.
SYMON_REAL_PYTHON = "${STAGING_BINDIR_NATIVE}/nativepython3"
SYMON_BUILD_HOME = "${@os.environ.get('HOME', '')}"

# Two generated artefacts recorded absolute build paths that no compiler flag
# can reach, failing do_package_qa [buildpaths]. symon_pytorch_write_build_hooks
# carries the evidence; these are the switches that arm the corrections.
export CMAKE_ARGS = "-DCMAKE_SYSTEM_NAME=Linux -DCMAKE_SYSTEM_PROCESSOR=x86_64 \
...
CMAKE_ARGS[vardepsexclude] = "SYMON_BUILD_HOME"

do_configure[prefuncs] += "symon_pytorch_write_build_hooks"

# ---------------------------------------------------------------------------
# do_package_qa [buildpaths]: two generated artefacts, two different layers.
#
# (A) caffe2/core/macros.h.  cmake/Codegen.cmake:46-48 runs an unconditional
#     configure_file() on caffe2/core/macros.h.in, whose CAFFE2_BUILD_STRINGS
#     table expands ${CMAKE_CXX_COMPILER} and ${CMAKE_CXX_FLAGS} verbatim.  The
#     compiler path is absolute and the flags string carries the text of OE's
#     own -ffile-prefix-map arguments, so the recipe work directory is written
#     into the shipped header and, once that header is #included, into
#     libtorch_cpu.so's .rodata.  -ffile-prefix-map cannot reach either: these
#     are CMake-expanded literals, not __FILE__ or DWARF.  The values are
#     diagnostic strings (caffe2::GetBuildOptions()), so rewriting the build
#     root to a fixed token keeps them meaningful and makes them byte-identical
#     across build roots.  The rewrite is deferred to the end of the configure
#     step - after Codegen.cmake has written the header, before anything
#     compiles - so the header and the library are corrected together.
#
# (B) third_party/NNPACK's PeachPy objects.  NNPACK/CMakeLists.txt:428-435 hands
#     an absolute .py path to PeachPy, and peachpy/formats/elf/image.py:19-20
#     stamps that argv string verbatim into an STT_FILE symbol.  No compiler is
#     involved, so no prefix-map flag applies; 12 build paths land in .strtab of
#     the -dbg package.  PeachPy never absolutises the argument
#     (peachpy/x86_64/__main__.py:243 -> writer.py:128 -> image.py:19), so
#     running it from ${S} with a source-root-relative input yields a
...
python symon_pytorch_write_build_hooks() {
    ...
    hooks = d.getVar("SYMON_HOOKS_DIR")
    bb.utils.mkdirhier(hooks)

    cmake_hook = """\
# Generated by symoneural-pytorch_git.bb - do not edit here.
# Injected through CMAKE_PROJECT_torch_INCLUDE, i.e. as the last step of the
# top-level project() call, so the deferred call below runs when the top-level
# directory has finished processing: after caffe2/CMakeLists.txt has included
# cmake/Codegen.cmake, and before the build step begins.

function(_symon_replace_prefix _var _from _to)
...
function(_symon_assert_absent _text _needle _where)
  if(NOT "${_needle}" STREQUAL "")
    string(FIND "${_text}" "${_needle}" _at)
    if(NOT _at EQUAL -1)
      message(FATAL_ERROR
        "symoneural: ${_where} still contains the build path ${_needle}; "
        "do_package_qa [buildpaths] would fail on it")
    endif()
  endif()
endfunction()

function(symoneural_sanitize_caffe2_macros)
  set(_h "${CMAKE_BINARY_DIR}/caffe2/core/macros.h")
  if(NOT EXISTS "${_h}")
    message(FATAL_ERROR
      "symoneural: ${_h} was not generated; the CAFFE2_BUILD_STRINGS build-path "
      "correction cannot be applied and must not be silently skipped")
  endif()
  file(READ "${_h}" _t)
  # Longest prefix first: WORKDIR lies under TMPDIR lies under HOME, so the
  # other order would leave fragments behind.
  _symon_replace_prefix(_t "${SYMON_BUILDPATH_WORKDIR}" "/workdir")
  _symon_replace_prefix(_t "${SYMON_BUILDPATH_TMPDIR}"  "/oe-tmpdir")
  _symon_replace_prefix(_t "${SYMON_BUILDPATH_HOMEDIR}" "/build-host-home")
  _symon_assert_absent("${_t}" "${SYMON_BUILDPATH_WORKDIR}" "${_h}")
  _symon_assert_absent("${_t}" "${SYMON_BUILDPATH_TMPDIR}"  "${_h}")
  _symon_assert_absent("${_t}" "${SYMON_BUILDPATH_HOMEDIR}" "${_h}")
  file(WRITE "${_h}" "${_t}")
  message(STATUS "symoneural: build paths removed from ${_h}")
endfunction()

cmake_language(DEFER CALL symoneural_sanitize_caffe2_macros)
"""
...
# Generated by symoneural-pytorch_git.bb - do not edit here.
#
# PeachPy writes its input path, exactly as given on the command line, into an
# STT_FILE symbol.  NNPACK passes an absolute path, which puts the recipe work
# directory into the -dbg package.  Re-express that one argument relative to
# the pristine source root and run from there.  Every other invocation - the
# -c probe FindPythonInterp uses, ordinary scripts - is passed straight
# through, so this stays a path correction and nothing more.
set -e
...
# NNPACK passes the .py input last.  Rotate argv so the last argument is $1,
# then drop it and re-append the relative form.
n=$#
i=1
while [ "$i" -lt "$n" ]; do
  set -- "$@" "$1"
...
case "$last" in
  "$SRCROOT"/*) ;;
  *)
    echo "symoneural: peachpy input '$last' is not under '$SRCROOT'; refusing" >&2
    echo "to emit a build path into an ELF FILE symbol" >&2
    exit 1
    ;;
esac
...
rel=${last#"$SRCROOT"/}
cd "$SRCROOT"
exec "$REAL_PYTHON" "$@" "$rel"
"""% {"python": d.getVar("SYMON_REAL_PYTHON"), "srcroot": d.getVar("S")}

    wrap_path = os.path.join(hooks, "peachpy-relative-path")
    with open(wrap_path, "w") as f:
        f.write(wrapper)
    os.chmod(wrap_path, 0o755)

    bb.note("symoneural: build hooks written to %s" % hooks)
}
